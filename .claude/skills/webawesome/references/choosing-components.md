...
"More…" button that opens a list of commands). For picking a value from a list, use `<wa-select>`.
This is the single most common confusion in the catalog.
...
**Callouts.** Use callouts for persistent messages that sit in the layout, such as form errors,
status panels, or information the user should not miss.
